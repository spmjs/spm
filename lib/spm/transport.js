...
try {
  compress = require('../../support/node-compress/compress');
  const GZIP = new compress.Gzip;
} catch (e) {
  GZIP = null;
}
...
function verifyVersion(version) {
  if (config.force || VERSION_REGEXP.test(version))
    return true;
...
function updateMeta(meta, min) {
  ...
  if (GZIP) {
    if (config.gzip) {
      GZIP.init();
      GZIP.deflate(fs.readFileSync(min));
      meta.gzipped = GZIP.end().length;
      console.log('gzipped size: %s', meta.gzipped);
    }
  } else {
    help.gzip();
  }
