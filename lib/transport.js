...
const ERRORS = {
    VERSION_EXISTS: function(src) {
        ...
        console.log('    if you want to upgrade your file to a new version.');
...
var handlers = {
    ...
    https: function(addr, fn) {
        require('https').get(addr, function(res) {
            var buffer = '';
            res.on('data', function(chuck) {
                buffer += chuck.toString();
            }).on('end', function() {
...
function getData(addr, fn) {
    ...
    } else if (addr.protocol === 'https:') {
        console.log('    getting content from %s..', addr.href);
        handlers.https(addr, fn);
    }

    // handle other protocol.
    else {
        console.warn('   unsupported protocol %s, ignore.', addr.protocol);
    }
}
...
function handleResult(meta, templ, data, suffix) {
    data = templ.split(PLACEHOLDER).join(data);
