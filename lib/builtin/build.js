...
function compress(settings, callback) {
  var encoding = settings.encoding || 'utf8', data, distfile, debugfile;
  Object.keys(settings.output).forEach(function(key) {
    distfile = path.join(settings.outputDirectory, key);
    if (/\.js$/.test(key)) {
      data = fs.readFileSync(distfile, encoding);
      debugfile = distfile.replace(/\.js$/, '-debug.js');
