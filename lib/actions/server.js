...
var argv = require('optimist').
    usage('Usage: $0 -p[server port] -d[relative dir] -r[root]')[
    'default']('p', '8000').
    argv;
...
console.info('req.url---->', req.url, req.method);
...
function getModulePath(moduleTarPath) {
  ...
  // 注册模块信息.
  registerModuleInfo(path.join(fileDir, modulePath, config.name), config);

  // 如果无法读取配置文件的root，默认启用在服务启动的时候配置.
  var modulePath = config.root == '#' ? '' : config.root || argv.r;
...
ModuleQueue.prototype.addQueue = function(modulePath, config) {
  var moduleName = path.basename(modulePath);
console.log('m--->', moduleName, config);
...
function registerModuleInfo(path, config) {
  MQ.addQueue(path, config);
}
