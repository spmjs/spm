...
console.log('getModulePath-->', tempModulePath);
  ...
  // 在根目录注册次root信息.
  if (root.indexOf('.') !== 0) {
    registerModuleInfo(fileDir, root);
  }
...
ModuleQueue.prototype.addQueue = function(modulePath, config, root) {
  root = root || '';
  var moduleName = path.join(root, path.basename(modulePath));
  ...
  if (!mq) {
    ...
    // 如果是注册root信息
    if (typeof config === 'string' && typeof info[config] === 'undefined') {
      info[config] = config;
    } else {
    ...
    mq.queue.drain = function() {
      ...
      // console.info('drain----->', util.inspect(mq.info));
      console.info('drain----->', moduleName, path.join(modulePath, 'info.json')); 
    }
