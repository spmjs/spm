...
var Config = require('../core/config_parse.js');
...
var loadConfig = function(cb) {
  ...
  config.addFile(path.join(process.cwd(), 'packge.json'));
...
var cacheConfig = function(cb) {
  var config = new Config();
  config.addFile(path.join(process.cwd(), 'config.json'));
  getProxyUrl('/config.json').forEach(function(proxy) {
    config.addUrl(proxy);
  });
  config.bind('end', function() {
  ...
  if (method === 'GET') {
    ...
    if (u === '/_reset') {
      ...
      cacheConfig(function() {
        console.info('reset config success!');
      }); 
    }
  } else {
    next(); 
...
function proxyModule(req, res, next) {
  ...
  if (!fsExt.existsSync(realPath)) {
    ...
    Sources.getValidUrls(requestUrl, function(sources) {
      if (sources.length === 0) {
