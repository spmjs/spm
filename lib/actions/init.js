...
var Init = ActionFactory.create('Init');
module.exports = Init;

Init.prototype.registerArgs = function() {
  var opts = this.opts;
  opts.description('create a empty module.');
  opts.option('-r --root [name]', 'default root value', String, '');
  opts.option('--base [dir]', 'set project directory.');
  opts.option('--module-type [module type]', 'select module template.');
};
...
Init.prototype.execute = function(options, callback) {
  ...
  var moduleType = options['module-type'];

  var searchStr = 'type=template';
  if (!moduleType && options.extras.length) {
...
function replaceTpl(src, dist, model) {
  ...
  var dataModel = _.clone(model);
  dataModel.name = Util.bigCamelStr(dataModel.name);
  data = Util.tmpl(data, dataModel);
