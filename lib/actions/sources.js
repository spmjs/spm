...
var Source = ActionFactory.create('Source');

Source.prototype.registerArgs = function() {
  var opts = this.opts;
  opts.help('upload module to source!\nusage: spm source [options]');
  opts.add('m', 'module', 'module name');
  opts.add('s', 'source', 'source path');
  opts.add('l', 'local', 'upload to local');
  opts.add('r', 'root', 'project root');
};
...
Source.prototype.parseDir = function(tempDir, moduleName) {
  var that = this;
...
Source.prototype.uploadTarToLocal = function(distPath, buildDir, tarName) {
