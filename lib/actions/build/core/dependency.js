...
Dependency.prototype.findGlobalModuleDeps = function(moduleName) {
  ...
  var isBaseModule = this.project.isBaseModule;
  ...
  if (isBaseModule) {
    ...
    // 如果当前项目是核心模块
    globalDistPath = this.project.repo;

    // 如果不是#打头,那么我们全部默认从arale or handy 里面加载，目前还不知道其他类型.
    ...
    // 检查项目依赖的是否是base module。
    if (isBaseModuleId(defineId)) {
      ...
      globalDistPath = this.project.baseRepo;
    } else {

      // 分析出项目类型, 然后从仓库中获取所依赖模块的地址.
      var parentType = getParentTypeByModId(defineId);
      if (parentType === this.project.parentType) {
        ...
        // 从全局配置中的仓库信息中寻找.
        globalDistPath = this.project.globalRepos[parentType].localDir;
      }
    }
  }
