...
function Project(action, projectDir) {
  ...
  var projectInfo = this.projectInfo = this.getProjectInfo();
  ...
  if (projectInfo.compress === 'closure') {
    this.compress = require('../compress/closure.js');
  }

  this.dist = projectInfo.dist || {};
  this.alias = projectInfo.alias || {};

  var globalAlias = this.getGlobalAlias();
  this.alias.__proto__ = globalAlias;
  this.aliasMobile = projectInfo['alias-mobile'];
