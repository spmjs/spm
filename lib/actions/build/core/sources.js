...
var Sources = module.exports = function(isLocal, project) {
  ...
  this.sourceDir = project.baseSourcePath;
};
...
Sources.prototype.getModuleSync = function(moduleId) {
  return this.getLocalSourceModuleSync(moduleId);
};
...
Sources.prototype.getLocalSourceModuleSync = function(moduleId) {
  var that = this;
  var moduleInfo = moduleIdParse(moduleId);
  var localPath;
  ...
  while (localPath = localPaths.pop()) {
console.log('localPath------>', localPath);
    if (fs.existsSync(localPath)) {
      return fsExt.readFileSync(localPath);
    }
  }

  throw 'Did not find the modules ' + moduleId + ' you need';
  return null;
};
...
Sources.prototype.getSourceModule = function(moduleId, callback) {
  ...
  var requestUrls = this.sources.map(function(source) {
    return source + '/' + path.join(moduleInfo.modulePath,
        moduleInfo.moduleTar);
  });

  async.detectSeries(requestUrls, function(requestUrl, callback) {

    request.head(requestUrl, function(err, res) {
      log('HTTP HEAD ' + res.statusCode + ' ' + requestUrl);
