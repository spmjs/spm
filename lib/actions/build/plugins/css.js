...
cssPlugin.param('convertStyle', 'none');
...
cssPlugin.run = function(callback) {
  ...
  modules = modules.filter(function(moduleName) {
    ...
    if (!cssModPattern.test(code)) {
      ...
      cssModPattern.exec(code, function(match, sep, mark, depModName) {
  ...
  queue.drain = function() {
    modules.forEach(function(moduleName) {
      var code;
      var codePath = path.join(project.build, moduleName);
...
function getReplacedImportStyleCode(moduleName) {
  var build = cssPlugin.project.build;
...
function getImportStyle(modName, depModName) {
  ...
  return 'seajs.importStyle(' + "'" + cssCode + "')";
}

function outputCssJs(modPath, depModName) {
  var cssModPath = path.join(cssPlugin.build, getDepModPath(modPath, depModName));
