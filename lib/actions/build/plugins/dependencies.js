...
deps.run = function(callback) {
  ...
  this.queue = async.queue(function(jsFile, callback) {
    that.parseJs(jsFile, callback);
  }, 1);
  ...
  var jsFile = fsExt.listFiles(this.build, /\.js$/).forEach(function(file) {
    that.queue.push(file);
  });
};

// 针对js进行替换.
deps.parseJs = function(filepath, callback) {
  var that = this;
  var project = this.project;
  var moduleCode = fsExt.readFileSync(filepath);
  var moduleFile = path.relative(this.build, filepath);
