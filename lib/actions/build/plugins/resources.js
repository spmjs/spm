...
var resources = module.exports = Plugin.create('resources', function(callback) {
  new ModuleHandler(this.project, function() {
 ...
 */
function ModuleHandler(project, callback) {
  var that = this;
  ...
  fsExt.listFiles(this.src).forEach(function(filepath) {
    that.queue.push(filepath);
  });
...
ModuleHandler.prototype.copy = function(filepath, callback) {
  var relaDir = path.relative(this.src, filepath);
  console.log('COPY FILE ' + relaDir);
  var moduleCode = fsExt.readFileSync(filepath);

  // 只替换模板.
  if (isJs(filepath)) {
    moduleCode = filterTplRequire(filepath, moduleCode, this.project);
  }
