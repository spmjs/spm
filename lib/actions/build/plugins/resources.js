...
// console.log(project.moduleDepMapping)
    console.log('');
    console.log(' The plugin resources completed successfully!');
    console.log('');
    callback();
  });
};
...
ModuleHandler.prototype.filterJs = function(filepath, callback) {
  ...
  async.waterfall([
    ...
    function(allDeps, callback) {
      ...
      log('RESOLVE DEPENDENCIES', '"' + moduleFile + '": ', allDeps.join(','));
...
ModuleHandler.prototype.resolveGlobalDeps = function(deps, callback) {
  deps = unique(deps);
log('BEGIN RESOLVE GLOBAL DEPENDENCIES ......', deps.join(','));
  var that = this;
  var project = this.project;
  var allDeps = [];
  var globalDeps = [];
...
//console.log(reg, eval(moduleCode.match(reg)[3]).length, 2222222);
  defineId = match[2];
...
console.log('parseDeps----->', defineId)
