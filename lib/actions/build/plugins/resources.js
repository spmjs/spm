...
ModuleHandler.prototype.filterJs = function(filepath, callback) {
  ...
  async.waterfall([
    function(callback) {
      ...
      that.resolveLocalDeps(path.dirname(relaDir), deps, resolvedDeps, callback);
    },
...
// parse所有依赖
ModuleHandler.prototype.resolveLocalDeps = function(modulePath, deps, resolved, callback) {
  var that = this;
//console.log('1110------>', modulePath, '3333', deps, resolved);
  deps = deps.slice(0);
...
console.log('depModuleName---->' + depModuleName)
    ...
    if (depModuleName.indexOf('.') === 0) {
      var filepath = path.join(that.src, modulePath, depModuleName + ".js");
      // 内部模块
      var moduleCode = fsExt.readFileSync(filepath); 
      var deps = parseDependencies(moduleCode);
      that.resolveLocalDeps(path.dirname(depModuleName), deps, resolved, function() {
