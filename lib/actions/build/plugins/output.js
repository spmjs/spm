...
//log('begin merge file :', output);
    ...
    console.log('');
    console.log(' The plugin merge completed successfully!');
    console.log('');
    callback();
  });
};
...
// log(filename, 'find rule------->', rule.id);
...
RuleHandler.prototype.merge = function(callback) {
  ...
  var handler = function(rule, filename, includes, callback) {
//log('rule handler merge-----', filename);
...
// console.log('oriDeps-->', include, deps);
      ...
      // 然后在用这些deps对代码进行替换.
      debugCode = filterLocalRequire(project, debugCode, '-debug');
...
function filterLocalRequire(project, code, debug) {
  var modPattern = project.getReqModRegByType('[^\"\']+');
  return code.replace(modPattern, function(match, mark, depModName) {
// log('filter local Require ----->', match, depModName);
      
    if (/(\.css|\.tpl)/.test(depModName)) {
      return match;
    }
...
var AllMergeRule = {
  ...
  getIncludes: function(project, filename, includes, callback) {
    var deps = project.getDepMapping(filename);
    var newDeps = deps.map(function(dep) {
      if (isRelative(dep)) {
...
var SeajsMergeRule = {
  ...
  getIncludes: function(project, filename, includes, callback) {
     AllMergeRule.getIncludes(project, filename, includes, function(allModule) {
