...
Build.prototype.execute = function(options, callback) {
  var that = this;
  ...
  var projectModel = ProjectFactory.getProjectModel(options, function(projectModel) {
    ...
    that._execute(projectModel, function(err) {
      if (err) {
        ...
        console.segm();
        console.info(action.toUpperCase() + ' SUCCESSFULLY!');
        console.segm();
      }
...
Build.prototype._parseOptions = function(options) {
  Object.keys(options).forEach(function(key) {
    ...
    switch (key) {
      ...
      case 'with-debug':
        ...
        options['debugName'] = value;
        delete options[key];
        break;

      case 'base':
        options['baseDirectory'] = perfectLocalPath(value);
        delete options[key];
        break;

      case 'to':
      case 'dist':
        options['distDirectory'] = perfectLocalPath(value);
        delete options[key];
        break;

      case 'build':
        options['buildDirectory'] = perfectLocalPath(value);
        delete options[key];
        break;
    }
  });
