...
Build.prototype.registerArgs = function() {
  ...
  opts.option('--only ', '仅执行当前指定阶段的插件.');
  opts.option('--global-config [global-config]', '加载全局配置, 支持 seajs.config');
  opts.option('--root [str]', '设置模块的 root [gallery]');
  opts.option('--base [base]', '设置模块的根目录, 默认当前目录', process.cwd());
  opts.option('--src [dir]', '设置模块源码目录, 默认 src');
  opts.option('--name [str]', '设置编译模块的名称');
  opts.option('--source [url]', '设置源服务地址. [modules.spmjs.org]');
  ...
  opts.option('--throwErrorOnDepNotFound [true or false]',
          '当依赖找不到的时候，是否终止执行, 默认是 false', false);
