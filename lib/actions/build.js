...
Build.prototype.registerArgs = function() {
  var opts = this.opts;
  opts.description('BUILD CMD MODULE');
  opts.option('--only [only]', 'only execute current phrase plugin.');
  opts.option('--global-config [global-config]', 'use user custom config.');
  opts.option('--root [str]', 'set module root.');
  opts.option('--base [base]', 'set base directory.', process.cwd());
  opts.option('--src [dir]', 'set src directory.');
  opts.option('--name [str]', 'set module name.');
  opts.option('--source [url]', 'set source server.');
  opts.option('--to [dir]', '设置模块输出目录.');
  opts.option('--ver [version]', 'set module version.');
  opts.option('--output [output config]', '设置模块合并规则.', parseOutput);
  opts.option('--source-files [dir]', 'set directory which need to compile.');
  opts.option('--throwErrorOnDepNotFound [true or false]',
          'throw error when dependencies not found.', false);
  opts.option('--with-debug [debugName]', 'add debug file.');
};
