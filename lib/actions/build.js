...
var SeaConfig = require('../utils/SeaConfig');
...
Build.prototype.run = function() {
  ...
  // spm build a.js --config app-config.js
  if (!options.base && options.config) {
    options.base = SeaConfig.parseBase(options.config, charset);
  }

  // make sure 'base' is root path.
  if (options.base) {
    options.base = path.resolve(cwd, options.base);
  }
