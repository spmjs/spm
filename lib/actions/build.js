...
Build.prototype.registerArgs = function() {
  ...
  opts.option('--compressor [str]', '指定压缩工具,默认 uglify [closure|uglify|yui]');
  opts.option('--compress-options [p1,p2,p3]', '设置压缩参数, 仅针对 yui 和 uglifyjs');
  opts.option('--skip-min', '跳过压缩');

  opts.option('--enable-coffee [boolean]', '是否启用 coffee-script 对代码进行编译');
  ...
  opts.option('--enable-less', '是否开启 less 编译. 默认开启.', parseBoolean);

  opts.option('--lint', '是否开启 jshint 或者 csslint 检查. 默认关闭', parseBoolean);
};
