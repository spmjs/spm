...
Install.prototype.execute = function(options, callback) {
  ...
  this.MDQ = new ModuleDownloadQueue(DEBUG, to, function() {
  ...
  var hasDeps = getDepsLength(path.join(base, 'package.json')) > 0;

  if (modules.length === 0 && hasDeps) {
    if (to == base) {
      to = path.join(base, 'sea-modules');
      this.MDQ.setTo(to);
    }
  }
  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      ProjectFactory.getProjectModel(that.createOptions(_options), function(model) {
         if (hasDeps) {
           var dependencies = model.dependencies;
           var reservedDeps = model.reservedDeps;

           modules = Object.keys(dependencies).filter(function(key) {
...
function getDepsLength(packageJsonPath) {
...
function parseModule(moduleId) {
  ...
  return idInfo.moduleName + '@' + idInfo.version; 
}
