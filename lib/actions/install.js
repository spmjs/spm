...
Install.prototype.run = function(callback) {
  ...
  this.MDQ = new ModuleDownloadQueue(argv['with-debug']);
...
//console.info('---->', codeDir, to, modId)
    // that.MDQ.parseDeps('#widget/1.0.0/widget', fsExt.readFileSync(path.join(to, 'widget.js')));
    callback();
  });
};
