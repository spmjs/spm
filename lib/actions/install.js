...
Install.prototype.getMeta = function(arg, registry, callback) {
  ...
  var BASE = this.options.from +
      [meta.name, meta.version, meta.filename].join('/');

  if (meta.min) meta.minUrl = BASE + '.js';
  if (meta.src) meta.srcUrl = BASE + '-debug.js';
...
Install.prototype.install = function(meta, callback) {
  ...
  // spm install name
  fsExt.mkdirS(path.dirname(to));
  fsExt.mkdirS(to);

  var BASE = path.join(to, meta.filename);
  ...
  if (meta.minUrl) {
    ...
    fsExt.readFromPath(meta.minUrl, function(code) {
      fs.writeFileSync(meta.minPath, code, 'utf8');
      done();
    });
  }
  ...
  if (meta.srcUrl) {
    ...
    fsExt.readFromPath(meta.srcUrl, function(code) {
      fs.writeFileSync(meta.srcPath, code, 'utf8');
      done();
    });
  }
