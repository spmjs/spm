...
Transport.prototype.run = function(callback) {
  var args = this.opts.argv._.slice(3);
...
console.info(args)
  ...
  var transportFile = path.resolve(args[0]);
  if (fs.statSync(transportFile).isDirectory()) {
  ...
  Transport.transport(transportFile, callback, this.opts.argv);
};


Transport.transport = function(transportFile, callback, argv) {
  ...
  Transport.getMeta(transportFile, function(err, meta) {
    ...
    if (fsExt.existsSync(minOutputFile) && !argv.force) {
...
Transport.getMeta = function(transportFile, callback, argv) {
