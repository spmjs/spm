...
Transport.transport = function(transportFile, callback, options) {
  ...
  Transport.getMeta(transportFile, function(err, meta) {
    ...
    function success() {
      ...
      echo(JSON.stringify(packageInfo)).to(path.join(path.dirname(srcOutputFile), 'package.json'));
      ...
      if (argv.upload || argv.deploy) {
        var baseDir = path.join(meta.root, meta.version);
...
function normalizeMeta(meta) {
  ...
  if (isRelative(meta.src)) meta.src = base + meta.src;
  if (isRelative(meta.min)) meta.min = base + meta.min;
...
function upload(model, cb) {
  ...
  tar(modPath, modName, ver, function() {
    var tarPath = path.join(modPath, ver, modName + '.tgz');
    uploadPlugin.uploadTarToServer(tarPath, model.getSource(), function() {
...
function tar(modPath, modName, ver, callback) {
  var modules = new Modules('', path.join(modPath, '..'), noop);

  modules.parseModule(modPath, modName, ver, function() {
