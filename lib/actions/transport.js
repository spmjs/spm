...
Transport.MESSAGE = {
  USAGE: 'Usage: spm transport [options] transport_file',
...
Transport.prototype.transport = function(transportFile, callback) {
  ...
  this.getMeta(transportFile, function(meta) {
    ...
    var name = (meta['filename'] || meta['name']).toLowerCase();
    ...
    function done() {
      getExtraFiles(meta['extra'], meta['src'], srcOutputFile, function() {
        // for unit test etc.
        callback && callback({
          'meta': meta,
          'tmpl': tmpl,
...
Transport.prototype.getMeta = function(filepath, callback) {
...
function getExtraFiles(extraFiles, remotePath, localPath, callback) {
  if (!extraFiles) {
    callback();
    return;
  }
  ...
  extraFiles.forEach(function(extraFile) {
    fsExt.readFromPath(extraFile, function(code) {
      var outputFile = path.join(localPath, extraFile.replace(remotePath, ''));
      fs.writeFileSync(outputFile, code, 'utf8');
      callback();
    });
  });
