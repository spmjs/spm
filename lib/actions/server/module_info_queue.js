...
InfoQueue.prototype.initModuleQueue = function() {
  ...
  this.queue = async.queue(function(task, callback) {
    ...
    info.versions[config.version] = that._initSubModules(config);
...
InfoQueue.prototype._initSubModules = function(config) {
  if (config.output) {
    return Object.keys(config.output);
  } else {
   var subModulesDir = path.join(this.rootPath,
         path.dirname(this.infoPath), config.version); 

   return fsExt.globFiles('**/*.js', subModulesDir).filter(function(f) {
     return !/^src/.test(f);                            
   });
  }
};
...
InfoQueue.prototype.initRootQueue = function() {
  ...
  this.queue = async.queue(function(task, callback) {
    ...
    if (info[name] && typeof config === 'string') {
      ...
      info[name] = config.versions || config;
      callback();
    }
  }, 1);
};
