...
plugin.param('skip-min', false, 'skip compress.');

var type;

plugin.run = function(project, callback) {
  if (this['skip-min'] || project.getConfig('skip-min')) {
  ...
  var compressor = project.getConfig('compressor') || this.compressor;
  ...
  if (sourceFiles && sourceFiles.length) {
    ...
    findDefaultMinJs(queue, sourceDir, debug);
    findDefaultMinCss(project.output, queue);
  }
};
...
function findDefaultMinCss(output, queue) {
