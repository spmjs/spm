...
deps.run = function(callback) {
  ...
  this.queue.drain = function() {
    console.info('----->', project.moduleDepMapping)
    callback();
  };
...
// 获取全局模块的依赖.
deps.findGlobalModDeps = function(project, globalDeps, allDeps, callback) {
  var that = this;
...
deps.findGlobalModuleDeps = function(moduleName, callback) {
  ...
  this.sources.getModule(defineId, function(err, moduleCode) {
    ...
    project.moduleDepMapping[defineId] = deps;
    callback(deps);
  });
};
...
var _depsCache = {};
deps.parseDependencies = function(moduleName) {
  moduleName = moduleHelp.normalize(moduleName);
  var deps = _depsCache[moduleName];
  if (deps) {
    return deps; 
  }

  var code = this.project.getModuleCode(moduleName);
  deps = depUtil.parse(code);
  deps = deps.filter(function(dep) {
    return !/tpl$|css$/.test(dep);
  });
...
deps.parseGlobalAsyncDependencies = function(code) {
  var ret = [], match;
  code = removeComments(code);
