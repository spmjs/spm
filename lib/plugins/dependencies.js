...
depPlugin.resolveLocalDeps = function(moduleName, resolved, callback) {
  if (this.resolvedModules[moduleName]) {
    callback();
    return;
  } else {
  ...
  async.forEachSeries(deps, function(depModuleName, callback) {
    ...
    // css 模块直接加入依赖，不进行后续处理.
    if (moduleHelp.isRelative(depModuleName) && /\.css$/.test(depModuleName)) {
    ...
    that.resolveLocalDeps(baseDepModuleName, resolved, function() {
      console.log('resolved------>', baseDepModuleName, resolved, depModuleName,moduleHelp.getBaseModule(baseDepModuleName));
