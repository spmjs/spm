...
deps.param('throwErrorOnDepNotFound', false, 'throw error when dependencies not found.');
...
deps.resolveGlobalDeps = function(deps, callback) {
  ...
  async.forEachSeries(globalDeps, function(dep, callback) {
    var depId = project.getGlobalModuleId(dep, that.throwErrorOnDepNotFound);

    // fix https://github.com/seajs/spm/issues/192
    if (!depId && that.throwErrorOnDepNotFound) {
