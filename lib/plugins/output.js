...
outputPlugin.run = function(project, callback) {
  ...
  extend_output(project, this.argv);
  ...
  // TODO 动态加载规则处理列表.
  handler.add(require('./output/rules/compositeMergeRule.js'));
  handler.add(require('./output/rules/jsMergeRule.js'));
  handler.add(require('./output/rules/cssMergeRule.js'));
  handler.add(require('./output/rules/resourcesRule.js'));
  handler.add(require('./output/rules/resourceDirRule.js'));
...
RuleHandler.prototype.normalizeOutputKeys = function() {
  ...
  Object.keys(output).forEach(function(o) {
    ...
    if (path.extname(o) === '' &&
      ...
      output[normalize(o, type)] = includes;
      delete output[o];
    }
  });
};
