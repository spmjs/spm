...
outputPlugin.parseOutput = function() {
  var project = this.project;
  argv = this.argv;
  var output = null;
  if (project.output) return project.output;
...
var jsOutput = function(ruleHandler, filename, includes, callback) {
  ...
  // debug 文件处理模块.
  var debugModule = new DebugModule(project, argv['with-debug']);
...
function DebugModule(project, debug){
  this.project = project;
...
function filterIdAndDeps(code, moduleId, deps) {
  ...
  return code;
  return code.replace(/define\s*\(\s*function/,
...
var SeajsMergeRule = {
  ...
  getIncludes: function(handler, filename, includes, callback) {
    ...
    AllMergeRule.getIncludes(handler, filename, includes, function(allModule) {
      var seajsId = project.getGlobalModuleId('seajs');
      if (seajsId) {
        allModule.unshift(seajsId);
      }
      callback(allModule);
    });
  }
