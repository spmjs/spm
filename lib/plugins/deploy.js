...
deployPlugin.deployToServer = function(project, key, server, callback) {
  ...
  var remotePath = path.join(server.path, createModPath(project));
...
deployPlugin.deployToLocal = function(project, key, config, callback) {
  ...
  var to = path.join(perfectLocalPath(config.path), createModPath(project));
...
function createModPath(project) {
  var idRule = project.getConfig('idRule');

  if (idRule) {
    return StringUtil.tmpl(idRule, {
      moduleName: ''
    });
  } else {
    return path.join(project.root, project.name, project.version);
  }
}
