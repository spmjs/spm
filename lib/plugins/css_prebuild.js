...
plugin.run = function(project, callback) {
  ...
  mergePaths.forEach(function(mergePath) {
    for(var len = mergePath.length; len > 0; len--) {
      mergeCssMod(mergePath[len], mergePath[len-1], merged, project);
    }
  });
...
// 检查冲突.
function mergeCssMod(subNode, parentNode, merged, project) {

  subNode = subNode || {};
  if (merged.indexOf(parentNode) > -1) {
    return;
  }

  var modName = parentNode.name;
  var cssCode = fsExt.readFileSync(modName);
  var hasDeps = false;

  cssCode = cssCode.replace(cssImportReg, function(match, depMod) {
    if (depMod.indexOf('.') == 0) {
      var subModPath = path.join(path.dirname(modName), depMod);
      if (subModPath == subNode.name) {

        hasDeps = true;
        merged.push(parentNode);
  ...
  if (hasDeps) {
    fsExt.writeFileSync(modName, cssCode);
  }
}
