...
plugin.run = function(project, callback) {
  ...
  var conflictMod = this._getConflictMod(reservedDeps, modDepMapping);

  if (conflictMod.length > 0) {
    var allDepLink = getAllDepLink(modDepMapping, reservedDeps);
    this._outputConflictedInfo(conflictMod, modDepMapping);
  }

  callback();
};

plugin._getConflictMod = function(reservedDeps, modDepMapping) {
...
plugin._outputConflictedInfo = function(conflictMod, allDepLink) {
