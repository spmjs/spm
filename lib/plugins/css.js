...
cssPlugin.run = function(project, callback) {
  ...
  var convertStyle = project.getConfig('convertStyle') || this['convert-style'];
  ...
  modules = modules.filter(function(moduleName) {
    ...
    if (!cssModPatternForCheck.test(code)) {
      ...
      var deps = depUtil.parseDynamic(depUtil.getAst(path.join(build, moduleName)));
      deps.forEach(function(depModName) {
        if (isCss(depModName)) {
...
function getReplacedImportStyleCode(moduleName) {
  ...
  var code = fsExt.readFileSync(path.join(build, moduleName));
  /**
  Ast.replaceRequire(code, isCss, function(depModName) {
    return getImportStyle(moduleName, depModName); 
  });
