  ...
  queue.drain = function() {
    modules.forEach(function(moduleName) {
      ...
      if (convertStyle !== 'none') {
        ...
        code = cssRequireCheck(getCssOutput(project.output), moduleName);
      }
...
function getCssOutput(output) {
  ...
  Object.keys(output).forEach(function(key) {
    if (moduleHelp.isCss(key)) {
      var value = output[key].slice(0);
      varlue = value.map(function(v) {
        if (v.indexOf('.') !== 0) {
          return './' + v;
        } else {
          return v;
...
function cssRequireCheck(cssOutput, moduleName) {
  ...
  return code.replace(cssModPattern, function(match, sep, mark, depModName) {
    if (cssOutput[depModName]) return match;

    Object.keys(cssOutput).forEach(function(key) {
      var value = cssOutput[key];
      var depModNamePath = moduleHelp.getBaseDepModulePath(moduleName, depModName);
      if (util.isArray(value) && value.indexOf(depModNamePath)) {
