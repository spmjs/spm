...
coffeePlugin.run = function(project, callback) {
  ...
  if (!this.enableCoffee && !project.getConfig('sourceFiles')) {
  ...
  var that = this;
  var dir = project.getConfig('sourceFiles') || build;
  ...
  if (haveCoffee) {
    ...
    var coffeeModPattern = project.getReqModRegByType('[^\"\']+\\.coffee', false);
