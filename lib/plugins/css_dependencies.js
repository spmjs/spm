...
var OMITTED_FOR_DUPLICATE = 1;

// State that represents a dependency node that has been omitted for conflicting with another dependency node.
var OMITTED_FOR_CONFLICT = 2;

// State that represents a dependency node that has been omitted for introducing a cycle into the dependency tree.
var OMITTED_FOR_CYCLE = 3;

var sources;
var project;

var Plugin = require('../core/plugin.js');
var plugin = module.exports = Plugin.create('css-dependency');

/**
 * 分析出来每一个 css 的依赖关系，然后保存在对应的数据结构
 * 在分析的时候需要检测循环依赖，如果有循环依赖抛出错误，主要是内部模块检查
 * a->b->c
 * 然后如果没有循环依赖，则开始合并。其中首先算出内部模块的依赖图，然后从出度为 0 的模块开始生成。然后边删除
 * 边生成模块，
