...
module.exports = function(nico) {
  ...
  var Denpendencies = getDependencies();
  ...
  exports.filters = {
    ...
    replace_code: function(content) {
      ...
      var value = util.format('%s/%s/%s', pkg.name, pkg.version, main.slice(main.lastIndexOf('/') + 1));
  ...
  exports.functions = {
    ...
    engines: function() {
      ...
      if (pkg.spm && pkg.spm.engines) {
        ...
        for (var d in Denpendencies) {
          var p = file.readJSON(path.join(Denpendencies[d], 'package.json'));
          var eng = p.spm.engines;
          for (var k in eng) {
            engines[k] = engines[k] || eng[k];
          }
        }
