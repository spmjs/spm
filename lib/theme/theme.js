...
module.exports = function(nico) {
  ...
  var dependencies = require('../sdk/module').getDependencies();
  ...
  function CovWriter() {}
  util.inherits(CovWriter, BaseWriter);
  CovWriter.prototype.run = function() {
    var option = nico.sdk.option;
    var dest = option.get('outputdir');
    mo.getSourceFiles().forEach(function(item) {
      if (path.extname(item) !== '.js') {
        item = item + '.js';
      }
      if (fs.existsSync(path.join(process.cwd(), item))) {
        processCovFile(item, dest);
      }
    });
  };
  nico.CovWriter = CovWriter;
    ...
    replace_code: function(content) {
      ...
      content = content.replace(scriptExp, function(replacement) {
        ...
        code = crequire(code, function(item) {
          var ret = item.path;
          if (ret.charAt(0) !== '.' && ret.indexOf('/') > -1) {
            ret = ret.split('/');
            if (dependencies[ret[0]]) {
              ret[0] = dependencies[ret[0]].split('/').slice(0, 2).join('/');
            }
            ret = ret.join('/');
          }
          if (ret.slice(-1) === '/') {
  ...
  function processCovFile(source, destDir) {
    var dest;
    if (/\.\w+$/.test(source)) {
    ...
    try {
      ...
      fs.writeFileSync(path.join(destDir, dest), changed);
    } catch(e) {
      log.error('error', 'generate cov file', file.to.red.color, 'with', e.toString().to.red.color);
    }
  }
};
