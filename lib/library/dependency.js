...
function transformDebug(ast) {
  ...
  var trans = new UglifyJS.TreeTransformer(function(node, descend) {
    if (node instanceof UglifyJS.AST_String) {
      ...
      } else if (parent instanceof UglifyJS.AST_Call && parent.start.value === 'define' && parent.args.length > 2) {
        return new UglifyJS.AST_String({
          start: node.start,
          end: node.end,
          value: node.getValue() + '-debug'
...
exports.transformDebug = transformDebug;
