...
var BaseCompiler = Class.create({
  ...
  write: function(filename, callback) {
    var buildir = getBuildDirectory(this.options);
  ...
  compile: function(callback) {
    ...
    if (depsInDefine) {
      ...
      logging.info(
        '[dependencies %s]: %s',
        filename,
        paint(depsInDefine).magenta.color
      );
    ...
    logging.info(
      '[dependencies %s]: %s', filename,
      paint(deps).magenta.color
    );
...
function getBuildDirectory(obj) {
  if (obj.buildDirectory) return obj.buildDirectory;
  if (!obj.configFile) return '.spm-build';
  return path.join(path.dirname(obj.configFile), '.spm-build');
}
