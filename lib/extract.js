...
function run(inputFile, outputFile, compress, depsOnly, baseFile) {
  inputFile = util.normalize(inputFile);
  baseFile = baseFile || path.join(process.cwd(), 'INDEX');

  var code = fs.readFileSync(inputFile, 'utf-8');
  var ast = uglifyjs.parser.parse(code);

  var info = extractInfo(inputFile, ast, baseFile, depsOnly);
...
function extractInfo(inputFile, ast, baseFile, depsOnly) {
  if (depsOnly !== false) {
    depsOnly = true;
  }

  var info = {};
  info.deps = getDependencies(ast);

  if (!depsOnly) {
    var name;

    // top-level modules
    if (inputFile.indexOf(modulesDir) === 0) {
      name = inputFile.substring(modulesDir.length);
    }
    else {
