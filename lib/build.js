...
function defineBuildTask(files, args) {
  ...
  files.forEach(function(file) {
    ...
    if (~['css', 'less', 'sass', 'styl'].indexOf(ext)) {
      gulp.task(name, ['install', 'clean'], buildCss(file, args));
      gulp.task(addDebug(name), ['install', 'clean'], buildCssDebug(file, args));
    } else {
      gulp.task(name, ['install', 'clean'], buildJs(file, args));
      gulp.task(addDebug(name), ['install', 'clean'], buildJsDebug(file, args));
    }
    deps.push(name);
...
function defineInstallTask(args) {
  if (args.install !== true) return gulp.task('install');
