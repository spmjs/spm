...
function defineBuildTask(files, args) {
  gulp.task('buildFiles', ['clean'], buildFiles(files, args));
  gulp.task('buildFilesDebug', ['clean'], buildFiles(files, args, true));
  gulp.task('build', ['buildFiles', 'buildFilesDebug']);
}

function buildFiles(files, args, isDebug) {
  ...
  return function() {
    return pipe(
      ...
      gulpif(isCSSorJS, pipe(
        transport(opt),
        gulpif(isCSSMin, cssmin()),
        // don't duplitate rename here
        gulpif(isStandalone, standalonify(args)),
        gulpif(isJS, pipe(
          gulpif(isDebug, beautify(beautifyOpt), uglify())
        ))
      )),
      gulp.dest(dest)
    );
  ...
  function isCSSMin(file) {
    return !isDebug && extname(file.path) === '.css';
  }
  function isJS(file) {
    return extname(file.path) === '.js';
  }
  function isStandalone(file) {
    return opt.include === 'standalone' &&
