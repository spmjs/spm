...
function checkDulp(file) {
  var dup = [];
  file.lookup(function(fileInfo) {
    // 不区分版本
    var filepath = fileInfo.pkg.name + '/' + fileInfo.filepath;
    if (dup.indexOf(filepath) === -1) {
      return dup.push(filepath);
    }
    log.warn('dulplicate', 'multiple version of `%s` has been required when building `%s`'.to.yellow.color, fileInfo.pkg.name, file.filepath);
  });
}
...
function buildFiles(files, args) {
  ...
  return function() {
    return pipe(
      ...
      $.if(isCSSorJS, pipe(
        $.mirror($.transport(opt), $.transport(optDebug)),
