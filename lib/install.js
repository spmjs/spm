 ...
 *  3. Copy the files is `dist` to `sea-modules/{name}/{version}/{file}
 */
...
function fetchCache(data, callback) {
  var fpath = path.join(config.cache, data.name);
...
function copy(src, callback) {
  ...
  file.recurse(dist, function(fpath) {
    ...
    // save alias to package.json
    save(fname, path.join(dest, fname), fpath);
  });
...
function save(alias, id) {
  ...
  id = id.replace(/\.js$/, '');
  id = id.replace(/^sea-modules\//, '');

  if (alias.indexOf('-debug') > 0) {
    return;
  }

  if (config.save) {
    log.info('alias saved', alias + ': ' + id);
    package.spm.alias = package.spm.alias || {};
    package.spm.alias[alias] = id;
  }
  if (config.saveDev) {
    log.info('devAlias saved', alias + ': ' + id);
    package.spm.devAlias = package.spm.devAlias || {};
    package.spm.devAlias[alias] = id;
  }
