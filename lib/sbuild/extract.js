...
function extractInfo(inputFile, baseFile, depsOnly, configFile) {
  var info = {};

  configFile = alias.getConfigFile(inputFile, configFile);
  info.deps = getDependencies(inputFile, configFile);
...
function getDependencies(inputFile, configFile) {
  var code = fs.readFileSync(inputFile, 'utf-8');
  var ast = uglifyjs.parser.parse(code);

  var deps = getAstDependencies(ast);
  if (configFile) {
