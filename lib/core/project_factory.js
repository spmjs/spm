...
Module.prototype = {
  ...
  _initGlobalInfo: function(modConfig) {
    ...
    this.version = argv.version || modConfig.version;

    // temp fix version is number.
  ...
  getGlobalModuleId: function(moduleName, slient) {
    ...
    if (!moduleId && !slient) {
      console.warn('not found dependency ' + moduleName);
      moduleId = moduleName;
    }
    return moduleId;
  },
...
function getPackageInfo(config) {
  ...
  var configCode = fsExt.readFileSync(config);
  ...
  try {
    ...
    if (path.extname(config) === '.json') {
      ...
      throw config + ' Is not a standard JSON file.';
    }
    var semiReg = /;\s+$/g;
...
function findParentModInfo(modDir) {
  ...
  while (parentDir !== '/') {
    ...
    if (fsExt.existsSync(parentConfig)) {
      var _modInfo = getPackageInfo(parentDir);
