...
function Module(modDir, callback) {
  ...
  modConfig.addFile(path.join(this.baseDirectory, CONFIG));
  ...
  modConfig.addFile(path.join(this.globalHome, SPM_CONFIG));

  modConfig.once('end', function(config) {
    that._initGlobalInfo(config._obj);

    // 查找那些是保留依赖.
    that._initResolvedDeps();
  
    that.moduleDepMapping = {};
    that.moduleSources = new Sources(this);
    callback(that);
  });
}
