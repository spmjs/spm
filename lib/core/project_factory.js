...
function Module(modConfig, callback) {
  ...
  PluginConfig.initUserPlugins(this, function() {
    callback(that);
  }); 
...
Module.prototype = {
  ...
  _initGlobalInfo: function(modConfig) {
    ...
    if (argv.build) {
      this.buildirectory = perfectLocalPath(argv.build);
    } else {
      this.buildDirectory = path.join(baseDir, '_build');
    }
  ...
  // 具体的插件根据相应的内容进行处理.
  // TODO 是否需要和src给默认关联起来?
  _parseSourceFiles: function() {
    var sourceFiles = argv['source-files'] || null;
    ...
    if (argv._.length > 3) {
      ...
      // spm lint src/**/*.js 
      // node会自动把通配符进行展开.
      sourceFiles = sourceFiles || argv._.slice(3);
    }

    if (typeof sourceFiles === 'string' && sourceFiles.indexOf(',') > -1) {
      // 解析source-files,目录计算出绝对路径, 文件拆分成对应的数组.
      sourceFiles = sourceFiles.split(',');
    }

    // TODO 如果包含通配符，需要对sourceFiles进行展开
    // 需要把*.js展开成文件列表.
  ...
  getModuleId: function(moduleName) {
    ...
    modId = path.join(this.name, version, moduleName.replace(extReg, ''));
  ...
  getModuleCode: function(moduleName) {
    return fsExt.readFileSync(this.buildDirectory, 
            help.normalizeJsModule(moduleName));
  },
