...
function Project(action, projectDir) {
    ...
    getGlobalAlias(baseDir, globalAlias);
...
// 
function getGlobalAlias(baseDir, alias) {
    
    var baseConfigStr = fsExt.readFileSync(baseDir,
            SPM_CONFIG);
    var baseAlias = parseAlias(baseConfigStr, alias);

}

// parse spm-config.js and get alias.
function parseAlias(spmConfigStr, alias) {
    ...
    Ast.walk(ast, 'stat', function(stat) {
        if (configFilter.is(stat)) {
            configFilter.parse(stat, config);
        }
        return stat;
    });

    pAlias = config.alias || {};
    alias.__proto__ = pAlias;
    for (var dep in config.deps) {
        if (config.deps.hasOwnProperty(dep)) {
            // 构建原形链
            return parseAlias(getServerConfig(dep), pAlias);           
        }
    }
}
...
function getServerConfig(dep) {
    ...
    // 4. 返回结果
    // HACK 暂时写死
    return fsExt.readFileSync('/Users/kanghui/Projects/alipay-project2/arale', SPM_CONFIG);
}
