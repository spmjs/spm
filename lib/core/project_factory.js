...
exports.getProjectModel = function(action, options, callback) {
  ...
  if (modConfig.isEnd()) {
    loadSourceConfig(modConfig, function() {
      new Project(options, modConfig, callback);
    });
  } else {
    modConfig.once('end', function() {
      loadSourceConfig(modConfig, function() {
        new Project(options, modConfig, callback);
      });
    });
...
function loadSourceConfig(modConfig, cb) {
  cb();
  return;
  ...
  async.forEachSeries(sources, function(source, cb) {
    if (help.isLocalPath(source)) return;

    var urls = [];
    var objs = {};

    source = help.perfectSource(source);
    var sourceUrl = source + '/' + SPM_CONFIG;
    var rootSourceUrl = root;
    objs[rootSourceUrl] = {};
    urls.push(sourceUrl);

    if (root) {
      rootSourceUrl = source + '/' + root + '/' + SPM_CONFIG; 
      urls.unshift(rootSourceUrl);
    }

    async.forEach(urls, function(u, cb) {
      _loadConfig(u, function(obj) {
        if (obj) {
          objs[u] = obj;
        }
        cb();
      ...
      ConfigParse.merge(objs[rootSourceUrl], objs[sourceUrl]);
      var roots = objs[rootSourceUrl].roots;
      if (roots && roots.indexOf(root) > -1) {
        modConfig.addConfig(objs[rootSourceUrl]);
      }
      cb();
    });
  }, function() {
...
function _loadConfig(configUrl, cb) {
  var config;
  var opt = {
    url: configUrl,
    timeout: 2000 //argv.timeout 
  };
...
Project.prototype = {
  ...
  _initDebugName: function() {
    ...
    if (_.isUndefined(this.debugName)) {
  ...
  getConfig: function(name) {
    ...
    var value = options.getConfig(name) || 
        options.getConfig(StringUtil.splitCamelStr(name));
