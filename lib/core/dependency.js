...
Dependency.prototype.findGlobalModuleDeps = function(moduleName) {
    ...
    var globalDistPath;
    var globalModuleDistPath;
    var globalId;

    if (defineId.indexOf('/') === 0) {
        // 判断是否是其他项目的依赖
        var projectType = defineId.slice(1, defineId.indexOf('/', 1));
        if (projectType === this.projectType) {
            // 是当前项目的依赖
            globalId = defineId.slice(globalId.indexOf('/', 1) + 1);
            globalDistPath = path.join(this.srcDir, '../../../dist');
        } else {
        ...
        if (this.project.projectType !== 'arale') {
            //TODO 目前依赖dist目录，后续会依赖服务器资源。
            globalDistPath = path.join(this.project.globalDeps.arale.local, 'dist');
        } else {
            globalDistPath = path.join(this.srcDir, '../../../dist');
        }
    }
    globalModuleDistPath = path.join(globalDistPath, globalId + '.' + this.project.type);
    var moduleCode = fsExt.readFileSync(globalModuleDistPath, '');
    var ast = astParser.parse(moduleCode);
    var deps = [];
    Ast.walk(ast, 'stat', function(stat) {
        if (stat.toString().indexOf('stat,call,name,define,string,' + defineId) !== 0) {
            return stat;
        }
