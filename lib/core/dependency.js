...
Dependency.prototype.parseLocalDeps = function(modAllDepMapping, modId) {
    ...
    while (subModName = local.shift()) {
        if (subModName.indexOf('#') === 0 || subModName.indexOf('~/') === 0 || subModName.indexOf('alipay') > -1) {
        ...
        if (subModuleDeps.length > 0 && (subModuleDeps[0].indexOf('#') === 0 || subModuleDeps[0].indexOf('alipay') > -1)) {
...
Dependency.prototype.findGlobalModuleDeps = function(moduleName) {
    ...
    if (defineId.indexOf('~/') === 0) {
    ...
    } else if (defineId.indexOf('alipay') > -1) {
        // 判断是否是其他项目的依赖
        ...
        if (this.project.parentType === 'handy') {
            globalDistPath = path.join(this.project.baseDir, 'dist');
        } else {
