...
Dependency.prototype.findGlobalModuleDeps = function(moduleName) {
    ...
    if (defineId.indexOf('~/') === 0) {
        // 判断是否是其他项目的依赖
        var projectType = defineId.slice(2, defineId.indexOf('/', 1));
        if (projectType === this.projectType) {
            // 是当前项目的依赖
            // ~/alipay/xbox/1.0/xbox ==> xbox/1.0/xbox
            globalId = defineId.slice(globalId.indexOf('/', 2) + 1);
    ...
    try{
        moduleCode = fsExt.readFileSync(globalModuleDistPath, '');
    } catch(e) {

        // 此错误不能接受.
        var err = 'Cannot access file(' + globalModuleDistPath +')!'
        throw new Error(err);
    }
