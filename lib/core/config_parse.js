...
Config.prototype = {
  ...
  // 添加本地文件. 允许用户传入处理函数.
  addFile: function(file, handler) {
    this._queue.push(function(callback) {
      var filepath = path.resolve(process.cwd(), file);
      this.addConfig(getConfigObj(fsExt.readFileSync(filepath), handler)); 
      callback();
    });
    return this;
  },
    ...
    this._queue.push(function(callback) {
      ...
      request(opts, function(err, res, body) {
        if (!err && (res.statusCode < 400)) {
          that.addConfig(getConfigObj(body, handler));
        } else {
...
// 获取模块配置信息.
function getConfigObj(configCode) {
...
function perfectLocalPath(localPath) {
  ...
  return path.normalize(process.cwd(), localPath);
}
