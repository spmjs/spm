...
var opts = require('../utils/opts.js').get();
var argv = opts.argv;
var home = env.home;
  ...
  setValue: function(key, value) {
    ...
    if (typeof baseValue === 'object' && typeof value === 'object') {
      Object.keys(value).forEach(function(subkey) {
        if (!_.has(baseValue, subkey)) {
          baseValue[subkey] = value[subkey];
        } else if (_.isObject(baseValue[subkey]) && _.isObject(value[subkey])) {
          // 如果存在，但是两个是对象，那么需要合并。后续可能要考虑更复杂的合并.
            _.defaults(baseValue[subkey], value[subkey]);
        } else {
          // 如果 value 存在，也不是对象，忽略。
        }
      });
    }
  },
