...
exports.parseCommands = function(options) {
  ...
  var data = {};
  if (options.cwd) {
    data.source = path.join(options.cwd, 'src');
    data.destination = path.join(options.cwd, 'dist');
  } else {
    data.source = 'src';
    data.destination = 'dist';
  }
  data.source = options.inputDirectory || data.source;
  data.destination = options.outputDirectory || data.source;
  return data;
};
...
exports.parseOptions = function(options) {
  ...
  var data = exports.parsePackage(pkg);
...
exports.parseDependencies = function(file, obj) {
  // almost everything in package.json should be in obj
  // required: root, name, version
...
function getRelativeDependencies(file, obj, basefile) {
  ...
  depsInRequire.forEach(function(name) {
    ...
    if (id.charAt(0) === '.' && /\.js$/.test(iduri.appendext(id))) {
