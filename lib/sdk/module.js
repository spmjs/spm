...
exports.parseCommands = function(options) {
  options = options || {};
  var data = {};
  if (options.inputDirectory) data.source = options.inputDirectory;
  if (options.outputDirectory) data.destination = options.outputDirectory;
  return data;
};

/* binding.spm */
exports.parseBinding = function(file) {
  file = file || 'binding.spm';
  if (!fs.existsSync(file)) return {};
  var data = fs.readJSONFileSync(file);
  if (!data.source) data.source = 'src';
  if (!data.destination) data.destination = 'dist';
  if (!data.builddir) data.builddir = '.spm-build';
  return data;
};
...
exports.parseOptions = function(options) {
  ...
  _.extend(data, exports.parseBinding(options.config));
