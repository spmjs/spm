...
Yuan.prototype.login = function(data, callback) {
  if (!(data.authkey && data.username)) {
    callback('Missing parameters.');
    return;
  }
  var req = {
    json: {account: data.username, authkey: data.authkey},
    method: 'POST',
...
Yuan.prototype.publish = function(data, callback) {
  ...
  req.urlpath = util.format(
    'repository/%s/%s/', data.name, data.version
  );
  req.method = 'POST';
  req.auth = auth;
  req.json = data;
...
Yuan.prototype.unpublish = function(data, callback) {
  var auth = spmrc.get(this.authKey);
  var req = {};
  req.urlpath = util.format('repository/%s/', data.name);
  if (data.version) {
