...
Yuan.prototype.request = function(data, callback) {
  ...
  // fix invalid argument: `family` must be 4 or 6
  // In node 0.11.x, data.family will be parsed by dns.js
  // https://github.com/joyent/node/blob/9a63a1d89b5a2a015eadd5ef5d7e42039858c5ae/lib/dns.js#L113
  delete data.family;
...
Yuan.prototype.publish = function(data, callback) {
  if (data.private && this.server === 'https://spmjs.org') {
...
Yuan.prototype.unpublish = function(data, callback) {
  var auth = spmrc.get(this.authKey);
  var req = {};
  req.urlpath = util.format('repository/%s/%s/', data.family, data.name);
  if (data.version) {
    req.urlpath += data.version + '/';
  }
  req.method = 'DELETE';
  req.json = true;
  req.auth = auth;
