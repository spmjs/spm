...
var BaseCompiler = Class.create({
  ...
  write: function(filename, callback) {
    var buildir = this.options.builddir || '.spm-build';
  ...
  getfilename: function() {
    var inputdir = this.options.source;
...
var JSCompiler = BaseCompiler.extend({
  ...
  compile: function(callback) {
    ...
    var deps = module.parseDependencies(this.filepath, obj);
    if (deps.length) {
...
var CopyCompiler = BaseCompiler.extend({
  ...
  run: function(callback) {
    var filename = this.getfilename();
    var dest = path.join(this.options.builddir, filename);
...
function getId(obj, filepath) {
  var input = obj.source || process.cwd();
