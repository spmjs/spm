...
// 对与这些模块我们不会自动添加后缀 .js
var FILE_EXT_RE = /\.(?:js|css|tpl|html|htm|coffee|less)$/;
...
exports.normalize = function(module, ext) {
  module = exports._normalizeModule(module, ext)
  return env.normalizePath(module);
};

// ./module/p.js ==> plugin/p.js
// ./module ==> module.js
exports._normalizeModule = function(module, ext) {
...
exports.getDebugModule = function(moduleName, debug) {
  ...
  if (isJs(moduleName) || isCss(moduleName)) {
    moduleName = moduleName.slice(0,
        moduleName.lastIndexOf(ext)) + debug;
  }
...
function getRequireByMatch(project, debug, match, sep, mark, depModName, req) {
  ...
  if (exports.isRelative(depModName)) {
    depModName = exports.getDebugModule(depModName, debug);
    return sep + req + "('" + depModName + "')";
  }
...
exports.globMods = function(mods, directory) {
  ...
  mods.forEach(function(mod) {
    if (!exports.isRelative(mod) && !/\*/.test(mod)) {
      _mods.push(mod); 
    } else {
