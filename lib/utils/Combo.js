...
Combo.compile = function(inputFile, outputFile, config) {
  ...
  // config: { charset, comboAll, libsPath }
  config || (config = {});
...
function getAllDependenciesFiles(filepath, config, ret) {
  ...
  deps.forEach(function(id) {
    ...
    if (config.combineAll || isRelativeId(id)) {
      var p = id;

      if (isRelativeId(id)) {
        p = path.join(basedir, id);
      }
      else if (isTopLevelId(id) && config.combineAll && LIBS_PATH) {
