...
module.exports = function(path, callback) {
  // 代码压缩.
  var code = fsExt.readFileSync(path);
  code = compress(code); 
  callback(code); // compressed code here
};

var compress = module.exports.compress = function(code) {
...
//console.info('args------>', args.shift(), options);
  out:while (args.length > 0) {
    ...
    if (key in compressorOptions) {
      compressorOptions[key] = true;
    }

    if (key in codegenOptions) {
      codegenOptions[key] = true; 
    }
  }
}
